...
from scipy.sparse import csr_matrix  # For sparse matrix handling
...
class EnsembleModel:
    def __init__(self):
        ...
        # Reduced feature set for memory optimization
    ...
    def fit(self, X_batch, y_batch):
        ...
        # Perform batch-wise transformation and sparsification
        X_transformed = csr_matrix(self.vectorizer.fit_transform(X_balanced)) if self.is_first_fit else csr_matrix(self.vectorizer.transform(X_balanced))
        logging.info(f"Transformed X_batch into sparse feature matrix of shape {X_transformed.shape}")
    ...
    def extract_features(self, X_batch):
        ...
        if isinstance(X_batch, str):
            X_batch = [X_batch]
        return csr_matrix(self.vectorizer.transform(X_batch))
